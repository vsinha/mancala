...
int eval_max (int board[], int player, int level, int *best_move, int alpha,
	      int beta);
int eval_min (int board[], int player, int level, int *best_move, int alpha,
	      int beta);
	  ...
	  if (board[p])		// if a valid move
	      ...
	      if (do_sort)	// fill choice[] with insertion sort
		{
		  ...
		  if (do_weighted_scoring)
		    {
		      tscore = tscore * 100 + sum (sort_board,
			                           P0POT + 1, P1POT - 1);
		    }
	  ...
	  if (board[p])
	    {
	      ...
	      if (move_again)
		{
		  this_eval = eval_max (eval_board, player, level,
		                        NULL, alpha, beta);
		}
	      else
		{
		  this_eval = eval_min (eval_board, !player, level - 1,
		                        NULL, alpha, beta);
		}
	      if (this_eval > alpha)
		{
...
int
eval_min (int board[], int player, int level, int *best_move, int alpha,
	  int beta)
{
  num_nodes += 1;
  if (level > 0 && !checkForWin (board))
	  ...
	  if (board[p])		// if a valid move
	      ...
	      if (do_sort)
		{
		  ...
		  if (do_weighted_scoring)
			...
			(tscore * 100) + sum (sort_board, 0, P0POT - 1);
		    }
		  while (k > 0 && choice[k - 1].score < tscore)
		    {
		      choice[k].score = choice[k - 1].score;
	  ...
	  if (board[p])
	    {
	      ...
	      if (move_again)
		{
		  this_eval = eval_min (eval_board, player,
		                        level, NULL, alpha, beta);
		}
	      else
		{
		  this_eval = eval_max (eval_board, !player,
		                        level - 1, NULL, alpha, beta);
		}
	      if (this_eval < beta)
		{
		  beta = this_eval;

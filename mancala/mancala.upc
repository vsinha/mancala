...
shared [] int shared_answers[NUM_POTS];
shared int shared_do_alpha_beta = 1;
shared int shared_do_sort = 1;
shared int shared_do_deepen = ;
...
int computer (int board[], int player)
{
	...
	if (work_board[MYTHREAD + P0POT + 1])
	{
	    move(work_board, player, MYTHREAD + P0POT + 1);
	    if (do_deepen)
	    {
		max_depth = 6;
		num_nodes = 0;
		do{
		  ...
		  num_nodes = 0;
		  (void) best_move(work_board, player, &best_score);
		  ++max_depth;
	...
	if (!MYTHREAD)
	{
	    ...
	    int i;
	    int chosen_move;
	    for(i = 0, best_score = INT_MIN; i < THREADS; ++i)
	    {
		if (shared_answers[i] > best_score)
		{
		    best_score = shared_answers[i];
		    chosen_move = i + P0POT + 1;
		}
	    }
	    fprintf(stats_file, "predict: %d | move: %d\n", best_score, chosen_move);
